...
import { WidgetsDemo } from '@/app/components/WidgetsDemo';
...
These widgets render through `OutputArea` with `isolated="auto"`, demonstrating the full isolation pipeline:

<div className="my-8 rounded border p-4" data-testid="widgets-demo">
  <WidgetsDemo />
</div>

The demo models raw Jupyter messages:
- `comm_open` creates widget models in the store
- Widget MIME type triggers iframe isolation
- `comm_msg` updates propagate through CommBridge to the iframe
