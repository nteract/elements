...
| `rendererCode` | `string` | — | React renderer JavaScript bundle |
| `rendererCss` | `string` | — | React renderer CSS |
...
The easiest way to load the bundle is via the `useIsolatedRendererBundle` hook:

```tsx
import { IsolatedFrame, useIsolatedRendererBundle } from "@/registry/outputs/isolated"
...
Use the `useIsolatedRendererBundle` hook to load the bundle, or import it directly via Vite's `?raw` import:

```tsx
// Option 1: Use the hook (recommended for apps serving the bundle)
const { rendererCode, rendererCss } = useIsolatedRendererBundle();

// Option 2: Import directly (for bundling into your app)
import rendererCode from "@/public/isolated/isolated-renderer.js?raw";
import rendererCss from "@/public/isolated/isolated-renderer.css?raw";
