...
<Callout type="info">
  MediaRouter depends on safe-to-render output components: AnsiOutput, ImageOutput, and JsonOutput. For HTML, Markdown, and SVG outputs (which require iframe isolation), use OutputArea or IsolatedFrame.
</Callout>
...
HTML, Markdown, and SVG outputs require iframe isolation for security (CSS attacks, forms, embedded content). MediaRouter returns `null` for these types and logs a warning in development.

For these MIME types, use OutputArea (recommended) or IsolatedFrame directly:

```tsx
import { OutputArea } from "@/components/cell/OutputArea"

// OutputArea automatically handles isolation
