...
CellContainer supports a **gutter ribbon** layout that creates a cleaner, paper-like aesthetic. Instead of borders, a thin colored ribbon on the left indicates cell type and focus state.
...
Enable gutter mode by passing a `cellType` prop:

```tsx
<CellContainer
  id={cell.id}
  cellType="code"           // Enables gutter mode
  isFocused={isFocused}
  gutterContent={           // Place play button in gutter
    <PlayButton gutterMode executionState={state} ... />
  }
>
  {/* Cell content - no CellHeader needed for play button */}
</CellContainer>
```

### Gutter Notebook Demo

Click cells to see the continuous ribbon effect:
...
| `cellType` | `string` | — | Enables gutter mode with ribbon coloring |
...
| `focusBgColor` | `string` | `"bg-primary/5"` | Legacy: Focus background |
| `focusBorderColor` | `string` | `"border-primary/60"` | Legacy: Focus border |
...
| `gutterMode` | `boolean` | Optimized visibility for gutter layout |
